...
def send_file(
    ip_addr, src_ip_addr="127.0.0.1", file_path="", max_packetsize=512, SLEEP=0.1
):
    """
    ...
    for i in range(1, x + 2):
        str_send = (
            IamDone[max_packetsize * (i - 1) : max_packetsize * i] + DATA_TERMINATOR
        )
    ...
    while True:
        ...
        if data[20] != "\x08":
            ...
            if filename != man_array[0]:
                sys.stderr.write(
                    "You tried transferring 2 files simultaneous. Killing my self now!\n"
                )
