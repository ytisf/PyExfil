...
class PDFCreator(object):
    ...
    def __init__(self, args, margins):
        ...
        self.linesPerPage = int(
            (self.leading + pageHeight - margins.top - margins.bottom - self.fontSize)
            / self.leading
        )
        ...
        if self.pageNumbering:
            self.pageNumberPlacement = (pageWidth / 2, margins.bottom / 2)
    ...
    def _process(self, data):
        ...
        for line in data:
            ...
            if sys.version_info.major == 2:
                read += len(line)
                yield flen == read, lineno, line.decode(self.encoding).rstrip("\r\n")
            else:
        ...
        with open(self.filename, "r") as data:
            for done, lineno, line in self._process(data):
                if len(line) > self.charsPerLine:
                    self._scribble(
                        "Warning: wrapping line %d in %s" % (lineno + 1, self.filename)
                    )
    ...
    def _writeChunk(self, page, chunk, lineno):
        if self.lineNumbering:
            ...
            for index, line in enumerate(chunk):
                page.textLine(formatstr % (lineno - len(chunk) + index + 1, line))
        else:
            for line in chunk:
                page.textLine(line)
...
parser.add_argument(
    "--font-size", "-s", type=float, default=10.0, help="Size of the font"
)
parser.add_argument(
...
parser.add_argument(
    "--media", "-m", default="A4", help="Select the size of the page (A4, A3, etc.)"
)
parser.add_argument(
    "--minimum-page-length",
    "-M",
...
parser.add_argument(
    "--margin-left", "-L", type=float, default=2.0, help="Left margin (in cm unit)"
)
parser.add_argument(
    "--margin-right", "-R", type=float, default=2.0, help="Right margin (in cm unit)"
)
parser.add_argument(
    "--margin-top", "-T", type=float, default=2.0, help="Top margin (in cm unit)"
)
parser.add_argument(
    "--margin-bottom", "-B", type=float, default=2.0, help="Bottom margin (in cm unit)"
)
...
parser.add_argument(
    "--page-numbers", "-n", action="store_true", help="Add page numbers"
)
...
PDFCreator(
    args,
    Margins(args.margin_right, args.margin_left, args.margin_top, args.margin_bottom),
).generate()
